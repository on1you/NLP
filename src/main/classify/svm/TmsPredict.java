...
//	}
	...
	public static void main(String[] args) {
		...
		int[][] indexes = {{1}};
		TmsPredict libsvm = new TmsPredict(paths);
		
		String in_filename = "D:\\svm\\svm_trainset\\test.txt"; //输入文件
		String tc_splitTag="\t"; //输入的数据各个部分之间的分割符号
		String str_splitTag ="\\^"; //标题和内容经过分词后，各个词的分割符号
		String str_connentTag="^";
		String out_filename = "D:\\svm\\result\\result.txt"; //结果保存的文件			
		...
		try {	
			...
			while(true){
				...
				for(SvmResult score :post_sc)
					output.print(score.getLabel()+"|"+score.getDescr()+"\t"+score.getScore());
